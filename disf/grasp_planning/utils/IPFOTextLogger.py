...
from disf.service import format_vector
from disf.value_object import IPFOParams
from disf.value_object import ISFResult
from disf.value_object import IPFOResult
...
class IPFOTextLogger:
    ...
    def dipfo_finished(self, result: IPFOResult):
        ...
        tag_color = "red"
        color = "magenta"
        print(f"---------------------------------------------------------", tag='DIPFO', tag_color=tag_color,
              color=color)
        print(f"       error = {format_vector(result.e_p_sum, decimal_places=4, format='decimal_exp')}", tag='DIPFO',
              tag_color=tag_color, color=color)
        print(f"      rotvec = {ExtendedRotation.from_matrix(result.R_sum).as_rotvec()}", tag='DIPFO',
              tag_color=tag_color, color=color)
    ...
    def palm_opt_finished(self, rotation_est, translation_est):
        if not self.ipfo: return
        print(
            f"palm optimization and update surface with rodrigues: rotvec={format_vector(rotation_est.as_rotvec())}, t={format_vector(translation_est)}",
    ...
    def isf_finished(self, result: ISFResult):
        ...
        print(f"                  error = {format_vector(result.error, decimal_places=4, format='decimal_exp')}",
              tag='ISF', tag_color='blue', color='blue')
        print(f" rotation (rotvec[deg]) = {format_vector(np.rad2deg(result.rotation.as_rotvec()), decimal_places=2)}",
              tag='ISF', tag_color='blue', color='blue')
        print(f" rotation (rotvec[rad]) = {format_vector(result.rotation.as_rotvec(), decimal_places=4)}", tag='ISF',
              tag_color='blue', color='blue')
        print(f"      rotation (quat)   = {format_vector(result.rotation.as_quat_scalar_first(), decimal_places=4)}",
              tag='ISF', tag_color='blue', color='blue')
        print(f"       translation      = {format_vector(result.translation, decimal_places=4)}", tag='ISF',
              tag_color='blue', color='blue')
    ...
    def gpa_finished(self, result_cma, result: ISFResult):
        ...
        print(f"             error = {format_vector(result.error, decimal_places=4, format='decimal_exp')}", tag='GPA',
              tag_color='green', color='green')
        print(f" rotation (rotvec) = {format_vector(result.rotation.as_rotvec(), decimal_places=4)}", tag='GPA',
              tag_color='green', color='green')
        print(f" rotation (quat)   = {format_vector(result.rotation.as_quat_scalar_first(), decimal_places=4)}",
              tag='GPA', tag_color='green', color='green')
        print(f"  translation_palm = {format_vector(result.translation_palm, decimal_places=4)}", tag='GPA',
              tag_color='green', color='green')
    ...
    def error_diff(self, error_diff):
        if not self.Rt_update: return
        print(f"---------------------------------------------------------", tag='Rigid_Transform', tag_color='yellow',
              color='yellow')
