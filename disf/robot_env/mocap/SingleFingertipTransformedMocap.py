import numpy as np
from disf.service import transform_gripper, direction_index
...
class SingleFingertipTransformedMocap:
    ...
    def __update_transform_into_base(self, gripper_transform_params: GripperTransformationParams):
        self.normal_world = np.dot(self.fingertip_base_mocap.normal_world,
                                   gripper_transform_params['rotation_matrix'].T)
        self.points_world_start = transform_gripper(points=self.fingertip_base_mocap.points_world_start, j=self.j,
                                                    **gripper_transform_params)
    ...
    def __update_transform_into_self(self, gripper_transform_params: GripperTransformationParams):
        ...
        self.points_world_start = transform_gripper(points=self.points_world_start, j=self.j,
                                                    **gripper_transform_params)
    ...
    def __update_mocap(self):
        for i in range(len(self.points_world_start)):
            self.mocap_manager.set_pos(mocap_body_id=self.mocap_manager.name2id(f"{self.mocap_name}_start{i}"),
                                       mocap_pos=self.points_world_start[i])
            self.mocap_manager.set_pos(mocap_body_id=self.mocap_manager.name2id(f"{self.mocap_name}_end{i}"),
                                       mocap_pos=self.points_world_end[i])
