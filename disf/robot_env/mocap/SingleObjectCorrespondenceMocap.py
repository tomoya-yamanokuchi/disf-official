...
class SingleObjectCorrespondenceMocap:
    ...
    def update_valid_points_and_normals(self):
        ...
        self.object_normal_world = self.object_mocap.normal_world[self.valid_target_indices]
        # --- fingertip (source) ----
        self.fingertip_points_world_start = self.fingertip_transformed_mocap.points_world_start[
    ...
    def __update_object_valid_mocap(self):
        for i in range(len(self.valid_target_indices)):
            # print(f"valid idx = {i}")
            self.mocap_manager.set_pos(mocap_body_id=self.mocap_manager.name2id(f"{self.object_mocap_name}_start{i}"),
                                       mocap_pos=self.object_points_world_start[i])
            self.mocap_manager.set_pos(mocap_body_id=self.mocap_manager.name2id(f"{self.object_mocap_name}_end{i}"),
                                       mocap_pos=self.object_points_world_end[i])
    ...
    def __update_fingertip_valid_mocap(self):
        for i in range(len(self.valid_source_indices)):
            # print(f"valid idx = {i}")
            self.mocap_manager.set_pos(
                mocap_body_id=self.mocap_manager.name2id(f"{self.fingertip_mocap_name}_start{i}"),
                mocap_pos=self.fingertip_points_world_start[i])
            self.mocap_manager.set_pos(mocap_body_id=self.mocap_manager.name2id(f"{self.fingertip_mocap_name}_end{i}"),
                                       mocap_pos=self.fingertip_points_world_end[i])
