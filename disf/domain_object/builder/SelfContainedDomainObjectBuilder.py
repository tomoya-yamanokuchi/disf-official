...
class SelfContainedDomainObjectBuilder:
    ...
    def build_model(self):
        import mujoco
        env_xml_path = os.path.join(PROJECT_DIR, self.config_env.xml)
    ...
    def build_body(self):
        from disf.robot_env.utils import BodyManager
        self.body = BodyManager(self.model, self.data)

    def build_site(self):
        from disf.robot_env.utils import SiteManager
        self.site = SiteManager(self.model, self.data)

    def build_geom(self):
        from disf.robot_env.utils.GeomManager import GeomManager
        self.geom = GeomManager(self.model, self.data)

    def build_camera(self):
        from disf.robot_env.utils.Camera import Camera
        self.camera = Camera(self.config_env, model=self.model)

    def build_option(self):
        from disf.robot_env.utils.Option import Option
        self.option = Option(config_viewer=self.config_env.viewer)

    def build_scene(self):
        from disf.robot_env.utils.Scene import Scene
    ...
    def build_config_env(self, config_name: str = "panda_hand"):
        from disf.config_loader import ConfigLoader
        # :::::::::::: domain object ::::::::::::
    ...
    def build_env(self):
        from disf.robot_env import RobotEnvFactory
    ...
    def build_pose(self):
        from disf.robot_env.instance.panda_hand.pose import PandaPoseFactory
    ...
    def build_ik_solver(self):
        from disf.robot_env.ik_solver import DampedLeastSquares
    ...
    def build_grasp_evaluator(self):
        from disf.mujoco_grasping import GraspEvaluation
    ...
    def build_ipfo_parameters(self):
        from disf.service import normalize_vector
        from disf.service import compute_object_qpos
        # :::::::::::: domain object ::::::::::::
        # ----------------------------------------
    ...
    def build_config_point_cloud_data(self, config_name: str):
        ...
        # ------------------------------------------------------
        self.output_file = os.path.join(PROJECT_DIR, self.config_pc_data.generate.output_file)
    ...
    def build_ycb_xml_scene_file(self, object_name: str):
        from disf.xml_generation import generate_mujoco_grasping_scene_with_ycb
        # ----
        abs_base_path_for_xml = os.path.join(PROJECT_DIR, self.pc_data_dir_path_relative_to_cwd)
    ...
    def build_ycb_target_by_contact_estimation_with_hand_plane(self):
        from disf.contact_detection import detect_plane_intersection
        from disf.value_object import PointNormalUnitPairs
    ...
    def shift_ycb_target_center_to_origin(self):
        from disf.service import shift_object_center_to_origin
    ...
    def build_single_fingertip_target_box(self, j: int):
        from disf.value_object import PointNormalCorrespondencePairs, PointNormalUnitPairs
    ...
    def build_point_cloud_dataset(self):
        from disf.value_object import PointNormalCorrespondencePairs
    ...
    def build_single_finger_indices(self, j: int):
        from disf.service import create_single_finger_indices
    ...
    def build_palm_least_square(self):
        from disf.grasp_planning.least_square import PalmPoseLeastSquares
        self.palm_least_square = PalmPoseLeastSquares(self)

    def build_coupling_palm_least_square(self):
        from disf.grasp_planning.least_square import CouplingPalmPoseLeastSquares
        self.coupling_palm_least_square = CouplingPalmPoseLeastSquares(self)

    def build_palm_approach_least_square(self):
        from disf.grasp_planning.least_square import PalmPoseApproachRLestSquare
        self.palm_approach_least_square = PalmPoseApproachRLestSquare(self)

    def build_disf_palm_R_ls(self):
        from disf.grasp_planning.disf.optimization.least_square import PalmRotationLeastSquare_EnEa
        self.disf_palm_R_ls = PalmRotationLeastSquare_EnEa(self)

    def build_disf_finger_ls_Ep(self):
        from disf.grasp_planning.disf.optimization.least_square import Point2PlaneLeastSquareWithFingertipDisplacement
        self.disf_finger_ls_Ep = Point2PlaneLeastSquareWithFingertipDisplacement(self)

    def build_palm_point2plane_least_square(self):
        from disf.grasp_planning.least_square import PalmPosePoint2PlaneLS
        self.palm_point2plane_least_square = PalmPosePoint2PlaneLS(self)

    def build_palm_normal_alignment_least_square(self):
        from disf.grasp_planning.least_square import PalmPoseNormalAlignmentLS
        self.palm_normal_alignment_least_square = PalmPoseNormalAlignmentLS(self)

    def build_finger_least_square(self):
        from disf.grasp_planning.least_square import FingertipDisplacementLeastSquare
        self.finger_least_square = FingertipDisplacementLeastSquare(self)

    def build_set_error(self):
        from disf.grasp_planning.utils.error_computation import SetErrorComputation
        from disf.grasp_planning.utils.error_computation import EpComputation
        from disf.grasp_planning.utils.error_computation import EnComputation
        from disf.grasp_planning.utils.error_computation import EaComputation
        # ----
    ...
    def build_com_error(self):
        from disf.grasp_planning.utils.error_computation import CoMError
    ...
    def build_visf_palm_Rt_opt(self):
        from disf.grasp_planning.visf.optimization import VISF_PalmOptimization
        self.visf_palm_Rt_opt = VISF_PalmOptimization(self)

    def build_visf_finger_opt(self):
        from disf.grasp_planning.visf.optimization import IPFO_FingertipDisplacementOptimization
        self.visf_finger_opt = IPFO_FingertipDisplacementOptimization(self)

    def build_visf_palm_Rt_EpEnEa_least_square(self):
        from disf.grasp_planning.visf.optimization.least_square import PalmLeastSquare_EpEnEa
        self.visf_palm_Rt_ls = PalmLeastSquare_EpEnEa(self)

    def build_visf_fingertip_displacement_least_square(self):
        from disf.grasp_planning.visf.optimization.least_square import FingertipDisplacement
    ...
    def build_isf_loop_with_single_icp(self):
        from disf.grasp_planning.disf import ISF_Planning_With_Single_ICP
        self.isf_planning = ISF_Planning_With_Single_ICP(self)

    def build_icp_matcher(self):
        from disf.correspondence_matching import ICPPointMatcherWithNormals
